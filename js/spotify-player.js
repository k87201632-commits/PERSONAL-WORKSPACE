...
function slBuildAddForm() {
    ...
    // Attach event setelah element dibuat
    // (requestAnimationFrame agar element sudah ada di DOM sebelum querySelector)
    requestAnimationFrame(() => {
        ...
        if (btn) {
            btn.addEventListener("click", () => slHandleAdd(nameEl, urlEl));
        }

        if (urlEl) {
            urlEl.addEventListener("keydown", (e) => {
                if (e.key === "Enter") slHandleAdd(nameEl, urlEl);
            });
        }
    });
...
function slBuildLibrarySection() {
    ...
    requestAnimationFrame(() => {
        ...
        if (clearBtn) {
            clearBtn.addEventListener("click", slHandleClearAll);
        }
    });
...
// HANDLER: TAMBAH ITEM
// --------------------------------------------------------------------------
function slHandleAdd(nameEl, urlEl) {
    ...
    if (msgEl) { msgEl.className = "sl-form-message"; msgEl.textContent = ""; }
...
function slBuildListItem(item) {
    ...
    li.querySelector(".sl-delete-btn").addEventListener("click", () => slDeleteItem(item.id));
...
function slDeleteItem(itemId) {
    ...
    if (wasPlaying) {
        ...
        const library = spotifyLibraryGet();
        if (library.length > 0) {
            slPlayItem(library[0]); // Putar item berikutnya
        } else {
            slShowDefaultPlayer();
        }
    }
